...
export async function saveMealRecord(formData: FormData) {
  ...
  const recordType = formData.get('recordType') as string; // 新增：紀錄類型 (personal 或 family)
  const personName = formData.get('personName') as string || null; // 新增：名字
  const personAge = formData.get('personAge') ? parseInt(formData.get('personAge') as string) : null; // 新增：年齡
